...
/**
 * An Auspice proxy is initialized around a key_directory, embodied as a directory tree whose
...
function reloadConfiguration (old_config, new_config) {
    _.each(old_config.whitelist.methods, function(value, key) {
        delete old_config[key];
    });

    _.each(old_config.whitelist.paths, function(value, key) {
        delete old_config[key];
    });

    // Copy the new keys into the existing keys object.
    _.extend(old_config.whitelist, new_config.whitelist);
}
    ...
    if (!keystore_reload_pending) {
      ...
      setTimeout(function() {
        ...
        this_obj._loadProxy(function(err, current_keys, current_config) {
          ...
          reloadConfiguration(this_obj.config, current_config);
        });
      }, 5000);
    }
  });

...
function parseAuthHeader(req) {
  ...
  if (req.headers.authorization) {
    ...
    if (auth_header.indexOf('OAuth') !== 0) return unauthorized('Authorization type is not OAuth');
...
function constructStringToSign(req, parameters, oauth_secret) {
  ...
  logger.trace("input url: %s", req.url);
  logger.trace("input headers: %s", util.inspect(req.headers));
...
exports.oauthValidator = function(keys) {
  return function(req, res, next) {
    try {
      if (!req || !req.headers) return unauthorized('Invalid request');
  
      var oauth_params = parseAuthHeader(req);
  
      logger.trace("Parsed auth header into:\n%s", util.inspect(oauth_params));
  
      if (oauth_params['oauth_signature_method'] !== 'HMAC-SHA1')
        return unauthorized('Only OAuth 1.0a with HMAC-SHA1 is supported');
      ...
      if (!oauth_params.hasOwnProperty('oauth_nonce') || !oauth_params.hasOwnProperty('oauth_timestamp') ||
         ...
         return unauthorized('Incomplete OAuth headers'); 
      ...
      if (isNaN(timestamp)) return unauthorized('Invalid oauth_timestamp');
      var now = new Date().getTime();
      if ((now - timestamp) > MAX_AGE) return unauthorized('Request expired, ' + (now - timestamp) + 'ms old');
  
      if (!keys.hasOwnProperty(oauth_params['oauth_consumer_key']))
        return unauthorized('Invalid consumer key');
      ...
      logger.error("Failed to handle request due to %s", e);
      res.writeHead(500);
      return res.end();
    }
  };

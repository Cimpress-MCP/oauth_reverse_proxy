...
Proxy.prototype.getConnectApp = function() {
  return this.config.isReverseProxy() ? this.getConnectReverseProxyApp() : this.getConnectProxyApp();
};
...
Proxy.prototype.getConnectProxyApp = function() {
  ...
  var oauth_param_generator = require('./mutators/oauth_param_generator.js')(this_obj);
  var query_string_parser = connect.query();
  var proxy_request_router = require('./mutators/proxy_request_router.js')(this_obj);
  ...
  return connect(
    ...
    // this in the future.
    form_parser,
    // Parse the inbound request to figure out its destination and desired key.  Rewrite the
    // request to include that information before signing and remove any control information
    // sent to the proxy.
    proxy_request_router,
    // Parse query string
    query_string_parser,
    // Parse url once so that it's available in a clean format for the oauth param generator.
    forward_header_mutator,
    // Validate that the request is within quota
    quota_validator,
    // Validate that the timestamp of the request is legal
    oauth_param_generator,
...
Proxy.prototype.getConnectReverseProxyApp = function() {
  ...
  // Mutators
  var auth_header_remover = require('./mutators/auth_header_remover.js')(this_obj);
  var form_parser = connect.urlencoded();
  var forward_header_mutator = require('./mutators/forward_header_mutator.js')(this_obj);
  ...
  return connect(
    ...
    host_header_mutator,
    // We have no need for the auth header and don't want to proxy it on to the target host, so strip it
    auth_header_remover,
    // Since connect messes with the input parameters and we want to pass them through
    // unadulterated to the target, we need to add restreamer to the chain.
    restreamer,

...
exports.requestValidator = function(proxy) {
  return function(req, res, next) {
    if (!req || !req.headers || !req.headers.host || !req.method || !req.url) {
    ...
    var check_collection = function(coll, comparison_string, error_message) {
      for (var i=0; i<coll.length; ++i) {
        var val = coll[i];
        //console.log('Checking %s against expected value %s', comparison_string, val);
        if (comparison_string.indexOf(val) !== -1) return true;
      }
    ...
    if (proxy.required_uris) {
      if (!check_collection(proxy.required_uris, req.url, 'Unmatched URI')) return false;
    }

    /* istanbul ignore else */
    if (proxy.required_hosts) {
      if (!check_collection(proxy.required_hosts, req.headers.host, 'Unmatched Host header')) return false;
    }

...
function JobServer() {
  ...
  ['GET', 'DELETE'].forEach(function(verb) {
    app[verb]("/job/:job_id", function(req, res) {
      ...
      console.log('%s with key %s', verb, req.headers['vp-auspice-consumer-key']);
  ...
  ['GET', 'POST', 'PUT', 'DELETE'].forEach(function(verb) {
    app[verb]('/compressed_content', function(req, res, next) {
      ...
      console.log('%s /compressed_content with key %s', verb.toUpperCase(), req.headers['vp-auspice-consumer-key']);
  ...
  ['GET', 'POST', 'PUT', 'DELETE'].forEach(function(verb) {
    app[verb]('/transactions', function(req, res) {
      ...
      res.setHeader('Content-Type', 'application/json');
      console.log('%s /transactions with key %s', verb.toUpperCase(), req.headers['vp-auspice-consumer-key']);
  ...
  app.POST('/getProducts', function(req, res) {
    console.log('POST /getProducts with key %s', req.headers['vp-auspice-consumer-key']);
  ...
  ['POST', 'PUT'].forEach(function(verb) {
    ...
    app[verb]('/uploads', function(req, res) {
      console.log('%s /uploads with key %s', verb.toUpperCase(), req.headers['vp-auspice-consumer-key']);

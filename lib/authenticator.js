  ...
  return function(req, res, next) {
    ...
    var check_collection = function(coll, comparison_string, error_message) {
      for (var i=0; i<coll.length; ++i) {
        ...
        if (comparison_string.indexOf(val) !== -1) return;
      }
    ...
    if (proxy.required_uris) {
      if (check_collection(proxy.required_uris, req.url, 'Unmatched URI')) return;
    }

    if (proxy.required_hosts) {
      if (check_collection(proxy.required_hosts, req.headers.host, 'Unmatched Host header')) return;
    }

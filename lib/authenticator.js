...
exports.requestValidator = function(proxy) {
  return function(req, res, next) {
    ...
    if (proxy.required_uris && !check_collection(proxy.required_uris, req.url, 'Unmatched URI')) {
      return false;
    }

    /* istanbul ignore else */
    if (proxy.required_hosts && !check_collection(proxy.required_hosts, req.headers.host, 'Unmatched Host header')) {
      return false;
    }
 ...
 */
exports.applyWhitelist = function(whitelist) {
  return function(req,res,next) {
    var passed = false;
    
    // If there is a methods block, let through anything with one of those methods
    if ("methods" in whitelist) {
      if (typeof whitelist.methods === "string" && whitelist.methods === req.method) {
        passed = true;
      }
      else if (Array.isArray(whitelist.methods) && whitelist.methods.indexOf(req.method) !== -1)  {
        passed = true;
      }
    }
    
    // If there is a paths block, see if any of those match the request...
    if (!passed && "paths" in whitelist) {
      if(Array.isArray(whitelist.paths)) {
        // Simple path matching is okay, I guess, and is an option
        //var current_path = _.findWhere(whitelist.paths, { path: req.parsed_url.pathname });
        var current_path = undefined;
        
        // Regex matching is slower but more powerful
        var possible_paths = _.filter(whitelist.paths, function(path_object) {
          var path_regex = new RegExp("^" + path_object.path + "$", "i");
          return (req.parsed_url.pathname.match(path_regex) !== null);
        });
        
        // If our path matched anything in the whitelist, just grab the first one.
        ...
        if (current_path !== undefined) {
          ...
          if ("methods" in current_path) {
            if(Array.isArray(current_path.methods) && current_path.methods.indexOf(req.method) !== -1) {
              passed = true;
            }
            else if(typeof current_path.methods === "string" && current_path.methods === req.method) {
              passed = true;
            }
          }
          else {
            // If the entry doesn't specify otherwise, implicitly allow all methods
            // (or should we?)
            passed = true;
          }
        }

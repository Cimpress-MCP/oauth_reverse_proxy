 ...
 */
exports.init = function(keystore_dir) {
  
  if (keystore_dir == undefined || !fs.existsSync(keystore_dir)) {
    logger.error("Failed to open directory %s", keystore_dir);
    process.exit(1);
  }
  
  // Traverse the key directory, validating the correct layout.
  interrogateRootDirectory(keystore_dir);
};
 ...
 */
function interrogateRootDirectory(keystore_dir) {

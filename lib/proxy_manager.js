...
module.exports.init = function(config_dir, cb) {
  if (!config_dir) throw new Exception("config_directory invalid");

  CONFIG_DIR = config_dir;

  var stat = fs.statSync(CONFIG_DIR);

  if (!stat.isDirectory()) throw new Exception('oauth_reverse_proxy config dir is not a directory');
...
function loadConfigFiles(cb) {
  ...
  fs.readdir(CONFIG_DIR, function(err, files) {
    ...
    var wrapped_cb = function() {
      ++required_calls;
      logger.warn("%s %s", required_calls, current_file);
      if (required_calls >= files.length) {
        var util = require('util');

        logger.trace("stop_me:\n%s", util.inspect(stop_me));
        logger.trace("start_me:\n%s", util.inspect(start_me));
        ...
        _.each(stop_me, function(proxy, proxy_config_file) {
          proxy.stop();
          delete this_obj.proxies[proxy_config_file];
        });
        ...
        logger.warn('hits === %s', required_calls);

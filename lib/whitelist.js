...
whitelist.prototype._passesMethodsWhitelist = function (method) {
    if ("methods" in this.config.whitelist) {
        if (typeof this.config.whitelist.methods === "string" && this.config.whitelist.methods === method) {
            return true
        }
        else if (Array.isArray(this.config.whitelist.methods) && this.config.whitelist.methods.indexOf(method) !== -1)  {
            return true
        }
    }
...
whitelist.prototype._passesPathsWhitelist = function (path, method) {
    if ("paths" in this.config.whitelist) {
        if(Array.isArray(this.config.whitelist.paths)) {
...
exports.applyWhitelist = function(config) {
    var whitelist_object = new whitelist(config);
    
    return function(req,res,next) {
        req.whitelist_passed = whitelist_object.applyWhitelist(req);

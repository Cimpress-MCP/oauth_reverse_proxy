...
// This header won't change during runtime of this process.
const DEFAULT_VIA_HEADER = '1.1 localhost (Auspice v' + process.env.AUSPICE_VERSION + ')';
...
// as they make their way through our infrastructure.  Set it with a UUID if it wasn't already set on the inbound request.
const CORRELATOR_ID_HEADER = exports.CORRELATOR_ID_HEADER = 'x-vp-correlatorid';
...
exports.applyCorrelationId = function() {
  return function(req, res, next) {
    ...
    // TODO: We may wish to write this in our responses, but that takes a bit more connect hackery than we would ideally
...
exports.modifyHostHeaders = function(from_port, to_port) {
  ...
  return function(req, res, next) {
    ...
    if (!host) {
      logger.warn('Seriously non-standard request headers seen: no host header');
      host = 'localhost';
    }

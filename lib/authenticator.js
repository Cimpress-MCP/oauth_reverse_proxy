...
const CONSUMER_KEY_HEADER = exports.CONSUMER_KEY_HEADER = 'x-vp-auspice-consumer-key';
 ...
 */
exports.requestValidator = function() {
  return function(req, res, next) {
    if (!req || !req.headers || !req.method || !req.url) return badrequest(req, res, 'Invalid request');
    next();
  };
};
    ...
    try {
      ...
      while (signature_bases.length > 0) {
        ...
        if (oauth_params[OAUTH_SIGNATURE] === hash) {
          ...
          logger.info("Proxying %s %s%s, consumer key %s, CorrelationId %s",
            req.method, req.headers.host, req.url, consumer_key, req.headers[CORRELATOR_ID_HEADER]);
          return next();
        }
      }

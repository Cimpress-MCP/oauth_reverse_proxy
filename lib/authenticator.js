...
function parseAuthHeader(req, cb) {
  ...
  if (req.headers.authorization) {
    ...
    for (var i=0; i<auth_header_parts.length; i+=4) {
      oauth_params[auth_header_parts[i].trim()] = auth_header_parts[i+2].trim();
    }
  }
...
function constructStringToSign(req, parameters, oauth_secret) {
  ...
  logger.info("input url: %s", req.url);
  logger.info("input headers: %s", util.inspect(req.headers));
  logger.info("url_parts:\n%s", util.inspect(url_parts));
...
exports.authenticateRequest = function(req, cb) {
  ...
  // Check date
  var timestamp = parseInt(oauth_params['oauth_timestamp']);
  if (isNaN(timestamp)) return cb('Invalid oauth_timestamp');
  var now = new Date().getTime();
  if (MAX_AGE > (now - timestamp)) return cb('Request expired');

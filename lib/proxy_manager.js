...
function create_proxy_config_finalizer(stop_me, start_me, cb) {
  ...
  return function() {
    ...
    _.each(start_me, function(proxy, proxy_config_file) {
      setProxyValue(path.basename(proxy_config_file), proxy);
  ...
  fs.readdir(CONFIG_DIR, function(err, files) {
    ...
    for (var i in files) {
      if (path.basename(files[i])[0] === '.') {
        ...
        config_files.push(path.resolve(CONFIG_DIR, files[i]));
      }
    }
    ...
    config_files.forEach(function(file) {
      ...
      fs.readFile(file, {'encoding': 'utf8'}, function(err, data) {
        ...
        try {
          ...
          // If this configuration has failed with the same error as last time we synced, remove
          // it from the list of proxies to stop.
          if (module.exports.proxies[file_name] === e.message) delete stop_me[file]

          setProxyValue(file_name, e.message);
          return wrapped_cb();
        }
        ...
        try {
          ...
          var existing_proxy = module.exports.proxies[file_name];
          ...
          // it from the list of proxies to stop.
          /* istanbul ignore next */
          if (module.exports.proxies[file_name] === msg) {
            delete stop_me[file_name];
          }

          /* istanbul ignore next */
          setProxyValue(file_name, msg);
          /* istanbul ignore next */
          wrapped_cb();
        }
      });

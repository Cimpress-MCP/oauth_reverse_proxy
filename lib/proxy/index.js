  ...
  var middleware_list = [
    {
      ...
      forward_proxy: true,
      reverse_proxy: true,
      value: body_parser.urlencoded({ extended: true })
    }, {
      name: 'json_parser',
      desc: '',
      forward_proxy: true,
      reverse_proxy: true,
      value: body_parser.json()
    }, {
...
Proxy.prototype.getConnectApp = function() {
  ...
  // Apply the appropriate middlewares via `connect()`.
  var app = connect();
  var middleware_list = getMiddlewareList(this, is_reverse_proxy);
  console.log('middleware_list ????', middleware_list);
  console.log('outside --> middleware_list[0].value ?????', middleware_list[0].value);
  console.log('outside --> middleware_list[0].value() ?????', middleware_list[0].value());
  // _.reduce(middleware_list, function(app_memo, middleware) {
  //   console.log('inside middleware ???', middleware);
  //   return app_memo.use(middleware.value);
  // }, app);

  _.each(middleware_list, function(middleware) {
    console.log('_.forEach inside middleware ???', middleware);
    app.use(middleware.value);
  });
  console.log('Proxy.prototype.getConnectApp app ???', app);

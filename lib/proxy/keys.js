      ...
      try {
        ...
        if (stat.isFile()) {
          ...
          if (!VALID_SECRET_PATTERN.test(file_contents)) {
            return logger.error("Invalid consumer secret pattern in file %s", file_name);
          }
          ...
          Object.defineProperty(this_obj, file_name, { value: encoding.encodeData(file_contents), writeable: false });
  ...
  fs.watch(this_obj.oauth_secret_dir, function() {
    if (!keystore_reload_pending) {
      ...
      setTimeout(function() {
        ...
        this_obj._loadProxy(function(err, current_keys) {
          /* istanbul ignore next */
          if (err) {

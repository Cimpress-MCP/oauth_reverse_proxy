...
var apply_correlation_id = header_modifier.applyCorrelationId();
...
var VALID_SECRET_PATTERN = /^[.-_a-zA-Z0-9]+$/;
    ...
    list.forEach(function(consumer_key) {
      try {
        ...
        if (stat.isFile()) {
          ...
          current_keys[consumer_key] = consumer_secret;
        } else {
 ...
 */
// Reinstate this when we resume automated key management.
/* istanbul ignore next */
function keySwap(keys, new_keys) {
  for (var key_name in keys) {
    if (keys.hasOwnProperty(key_name)) {
    ...
    if (!keystore_reload_pending) {
      ...
      setTimeout(function() {
        ...
        this_obj._loadKeys(function(err, current_keys) {
          if (err) {
            return logger.error("Failed to load keys at proxy startup.  %s", err);
          }
...
authenticatingProxy.prototype.start = function(cb) {
  ...
  this_obj._loadKeys(function(err, current_keys) {
    ...
    // If no keys were found, fail proxy creation.  NOTE: Remove this test when we reinstate key watching.
    if (_.isEmpty(current_keys)) {
      return cb('No keys found.');
    }
    ...
    this_obj.server = connect.createServer(
      ...
      url_parser,
      // Modify the request headers to add x-forwarded-* and vp-correlation-id
    ...
    // NOTE: We do not need the key watcher until we have a robust solution for moving keys
    // around the network.  Disabling this until then since it's not a mature feature.

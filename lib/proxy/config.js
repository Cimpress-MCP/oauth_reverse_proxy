...
function ProxyConfig(config) {
    ...
    // If the specified keystore path is not absolute, then assume it's location is relative to the top-level
    // directory of this node module.
    var resolved_oauth_secret_dir = path.resolve(__dirname, '../../', config.oauth_secret_dir);
    ...
    Object.defineProperty(this_obj, 'oauth_secret_dir', { 'value': resolved_oauth_secret_dir, writable: false, enumerable: true });
...
ProxyConfig.prototype.isInvalid = function() {
    ...
    if (typeof this.oauth_secret_dir === 'undefined' || !fs.lstatSync(this.oauth_secret_dir).isDirectory()) {
      return "oauth_secret_dir must exist";
    }

...
Proxy.prototype.start = function(cb) {
  ...
  this_obj.keystore.load(function(err) {
    ...
    var proto = this_obj.config.https === true ? https : http;

    // IPv4 server
    proto.createServer(app).listen(this_obj.config.from_port, '0.0.0.0');

    // IPv6 server
    proto.createServer(app).listen(this_obj.config.from_port, '::');

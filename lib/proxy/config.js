...
ProxyConfig.prototype.isInvalid = function() {
    ...
    if (this.quotas) {
      if (this.quotas.interval) {
        ...
        if (isNaN(interval)) return "quota.interval must be a number";
        if (interval < 1) return "minimum quota.interval is 1 second";
      }

      if (this.quotas.default_threshold) {
        var default_threshold = parseInt(this.quotas.default_threshold);
        if (isNaN(default_threshold)) return "quota.default_threshold must be a number";
        if (default_threshold <= 0) return "quota.default_threshold must be positive";
      }

      if (this.quotas.thresholds) {
        for (var key in this.quotas.thresholds) {
          var threshold = parseInt(this.quotas.thresholds[key]);
          if (isNaN(threshold)) return key + " threshold must be a number";
          if (threshold <= 0) return key + " threshold must be positive";
        }
      }
    }

...
// We use string trimming as a convenience in this module, so define it here.  We use the
// name vp_authenticator_trim in the off-chance that some other module or dependency defines
// an incompatible trim method.
if (typeof(String.prototype.vp_authenticator_trim) === "undefined") {
  // Add a strip command to make input parsing cleaner.
  String.prototype.vp_authenticator_trim = function() {
...
function gatherSignableParams(req, res, oauth_params) {
  ...
  if (req.headers.authorization) {
    ...
    for (var i=0; i < auth_header_parts.length; i+=4) {
      var param_name = auth_header_parts[i].vp_authenticator_trim();
      if (param_name === 'realm') continue;
      var param_value = auth_header_parts[i+2].vp_authenticator_trim();
...
function validateTimestamp(oauth_params, req, res) {
  ...
  var valid = ((now - timestamp) < MAX_AGE);
  if (!valid) {

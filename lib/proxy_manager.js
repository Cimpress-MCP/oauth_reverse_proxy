    ...
    config_files.forEach(function(file) {
      ...
      fs.readFile(path.resolve(CONFIG_DIR, file), {'encoding': 'utf8'}, function(err, data) {
        ...
        try {
          ...
          // Also ensure that the specified proxy keystore directory is an absolute file
          // path, resolved with respect to the application CONFIG_DIR.
          var config = JSON.parse(data);
          if(typeof config.oauth_secret_dir !== 'undefined' && config.oauth_secret_dir !== '')
            config.oauth_secret_dir = path.resolve(CONFIG_DIR, config.oauth_secret_dir);
